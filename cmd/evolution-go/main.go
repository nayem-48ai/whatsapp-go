...
func setupRouter(db *gorm.DB, authDB *sql.DB, sqliteDB *sql.DB, config *config.Config, conn *amqp.Connection, exPath string, runtimeCtx *core.RuntimeContext) *gin.Engine {
	...
	// Self-hosted license server (WhatsappGo): issues licenses from your own
	// database when LICENSE_MODE=self. No-op otherwise.
	core.SelfLicenseRoutes(r)
...
// @title WhatsappGo
// @version 1.0
// @description WhatsappGo - whatsmeow
func main() {
	flag.Parse()
	if *devMode {
	...
	logger.LogInfo("Starting WhatsappGo version %s", version)
